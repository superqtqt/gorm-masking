...
func (m *Masking) Initialize(db *gorm.DB) error {
	...
	RegisterTypeOptions(Common, NewCommonMasking(m.config.Key, float32(0.5), Middle))
	...
	for k, _ := range fieldName {
		...
		if desensitizationType, ok := v.Tag.Lookup(MaskingTag); ok {
			if v.FieldType.Kind() != reflect.String {
				panic(fmt.Sprintf("struct=%s proertty=%s masking field type must be string", db.Statement.Schema.Name, k))
			}
